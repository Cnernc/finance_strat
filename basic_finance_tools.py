...
def get_or_download_data(tickers, start_date, end_date, price_types=['Open'], directory='stock_data') -> pd.DataFrame:
    ...
    file_name = f"{tickers}_{start_date}_to_{end_date}.csv"
...
def axial_grid_search(f:callable, param_dict:dict, param_selected:str):
    ...
    :param param_selected: The parameter to vary.
    :param f: Callable function for evaluation (e.g., evaluate_OLS_strat).
    """
    param_1_values = param_dict[param_selected]
    ...
    for i, param_1 in enumerate(param_1_values):
        best_params, best_result = grid_search(f, {**{param_selected: [param_1]}, **param_dict_cut})
        best_params_values.append(best_params)
        best_result_values.append(best_result)
        eff_result_values.append(f(**{**best_params, **{'metric': 'effective_sharpe_ratio'}}))
        print(i+1, '/', len(param_1_values), end=' ')
    print('\n')
